...
const ORIGIN_CLASS: Record<ClientOrigin, string> = {
  pedido: 'text-amber-400 bg-amber-500/10 border-amber-500/20',
  importacao_manual: 'text-violet-400 bg-violet-500/10 border-violet-500/20',
  cadastro_administrativo: 'text-sky-400 bg-sky-500/10 border-sky-500/20',
  outro: 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20',
};
...
export default function ClientsList() {
  ...
  const [editForm, setEditForm] = useState({
    name: '', phone: '', stamps: '0', cashback: '0',
    origin: 'cadastro_administrativo' as ClientOrigin, notes: '',
  });
  ...
  const saveEdit = async () => {
    ...
    await run(edit.id, async () => {
      await updateClient(edit.id, {
        ...
        stamps: Math.max(0, parseInt(editForm.stamps || '0', 10) || 0),
        cashbackBalance: Math.max(0, parseFloat(editForm.cashback.replace(',', '.') || '0') || 0),
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-zinc-600 text-xs">CRM · {clients.length} cadastros</p>
            </div>
          </div>
            ...
            {clients.map((c) => (
              <article key={c.id} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="text-zinc-600 text-[11px] mt-0.5">
                      Cadastro: {formatDate(c.joinedAt || c.createdAt)}
                      {' · '}Pedidos: {c.orderCount}
                      {' · '}Último: {formatDate(c.lastOrderAt)}
                    </p>
                    {c.notes ? (
                      <p className="text-zinc-500 text-[11px] mt-1 line-clamp-2">{c.notes}</p>
                    ) : null}
                  </div>
                  <span className={`shrink-0 text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${ORIGIN_CLASS[c.origin] || ORIGIN_CLASS.outro}`}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  <button type="button" disabled={busyId === c.id}
                    onClick={() => {
                      if (!confirm(`Excluir ${c.name}? Os pedidos do cliente não serão apagados.`)) return;
          ...
          <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-950 p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                ...
                <Input type="number" min={0} value={editForm.stamps} onChange={(e) => setEditForm((f) => ({ ...f, stamps: e.target.value }))}
                  className="bg-zinc-900 border-zinc-800 text-white h-11" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-zinc-400 text-xs">Cashback</Label>
                <Input type="number" min={0} value={editForm.cashback} onChange={(e) => setEditForm((f) => ({ ...f, cashback: e.target.value }))}
                  className="bg-zinc-900 border-zinc-800 text-white h-11" />
              </div>
            </div>
            <div className="space-y-1.5">

...
export default function ClientsImport() {
  ...
  const handleSave = async (updateIfExists = false) => {
    setSaving(true); setError(''); setSuccess('');
    if (!updateIfExists) setExisting(null);
    try {
      const result = await createClient({ ...payload(), updateIfExists });
      if (!result.ok) {
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <p className="text-zinc-500 text-xs leading-relaxed">
            ...
            O WhatsApp é o identificador principal — números iguais nunca geram duplicata.
          </p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-4">
          ...
          {existing && (
            <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 space-y-2">
              ...
              <p className="text-zinc-500 text-xs">
                {existing.stamps} selos · R$ {parseFloat(existing.cashbackBalance || '0').toFixed(2).replace('.', ',')} cashback
                · {existing.orderCount} pedidos
              </p>
          ...
          <Button onClick={() => void handleSave(false)} disabled={saving || !name.trim() || phone.replace(/\D/g, '').length < 10}

...
/** Keep both icons mounted — swapping component types mid-tree can trigger insertBefore crashes. */
function SaveGlyph({ saving }: { saving: boolean }) {
  return (
    <span className="relative inline-flex h-[18px] w-[18px] shrink-0 items-center justify-center">
      <Loader2
        size={18}
        className={`absolute animate-spin transition-opacity ${saving ? 'opacity-100' : 'opacity-0'}`}
        aria-hidden={!saving}
      />
      <Check
        size={18}
...
function ToggleGlyph({ on, disabled }: { on: boolean; disabled?: boolean }) {
  return (
    <span className={`relative inline-flex h-[34px] w-[34px] items-center justify-center ${disabled ? 'opacity-40' : ''}`}>
      <ToggleRight
        size={34}
        className={`absolute transition-opacity ${on ? 'opacity-100' : 'opacity-0'}`}
        aria-hidden={!on}
      />
      <ToggleLeft
        size={34}
        className={`absolute transition-opacity ${on ? 'opacity-0' : 'opacity-100'}`}
        aria-hidden={on}
      />
    </span>
  );
...
function AdminNav({ active }: { active: string }) {
  const items = [
    { href: '/admin', icon: TrendingUp, label: 'Início' },
  ...
  return (
    <div className="space-y-4">
      ...
      <Button onClick={handleSave} disabled={saving}
        ...
        <SaveGlyph saving={saving} /> Salvar Tempo de Preparo
      </Button>
    </div>
  );
}
...
function PaymentTab() {
  ...
  const applySettings = (s: PaymentSettingsAdmin) => {
    setSettings(s);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (alive) setError('Não foi possível carregar as configurações');
      } finally {
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const updated = await updatePaymentSettings({
        clearMercadoPagoCredentials: true,
  ...
  const mpConfigured = !!settings.mercadoPagoConfigured;
  const pixConfigured = !!settings.pixConfigured;

  return (
    <div className="space-y-5">
      {/* Static Pix — active now */}
      <div className={`rounded-2xl p-5 border flex items-start gap-3 ${pixConfigured ? 'bg-green-900/10 border-green-800/40' : 'bg-amber-900/10 border-amber-800/40'}`}>
        <span className="relative mt-0.5 shrink-0 inline-flex h-[22px] w-[22px]">
          <ShieldCheck size={22} className={`absolute text-green-400 ${pixConfigured ? 'opacity-100' : 'opacity-0'}`} aria-hidden={!pixConfigured} />
          <ShieldAlert size={22} className={`absolute text-amber-400 ${pixConfigured ? 'opacity-0' : 'opacity-100'}`} aria-hidden={pixConfigured} />
        </span>
        <div>
          <p className={`font-bold text-sm ${pixConfigured ? 'text-green-400' : 'text-amber-400'}`}>
            {pixConfigured ? 'Pix manual ativo' : 'Configure a chave Pix'}
          </p>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 space-y-3">
        ...
        <div className="space-y-1.5">
          <Label className="text-zinc-500 text-xs">
            Access Token{' '}
            <span className={`text-zinc-600 ${mpConfigured ? '' : 'invisible'}`}>
              (atual: {settings.mercadoPagoAccessTokenPreview || '••••'})
            </span>
          </Label>
          <Input type="password" value={accessToken} onChange={e => setAccessToken(e.target.value)}
            placeholder={mpConfigured ? 'Deixe em branco para manter o token atual' : 'APP_USR-...'}
            className="bg-zinc-950 border-zinc-800 text-white h-11 text-sm focus:border-amber-500 font-mono" />
        </div>
        <div className="space-y-1.5">
        ...
        {/* Always mounted — visibility only via CSS (avoids insertBefore on first credential save). */}
        <button
          type="button"
          onClick={handleDisconnect}
          disabled={saving || !mpConfigured}
          className={`text-red-400 text-xs font-bold hover:underline text-left ${mpConfigured ? '' : 'invisible pointer-events-none'}`}
        >
          Remover credenciais
        </button>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-zinc-500 text-xs mt-0.5">
              {mpConfigured ? 'Processa Pix e cartão automaticamente via Mercado Pago' : 'Salve as credenciais acima para poder ativar'}
            </p>
          </div>
          <button
            type="button"
            ...
            className={`transition-colors ${!mpConfigured ? 'text-zinc-700 cursor-not-allowed' : online ? 'text-amber-500' : 'text-zinc-600'}`}
          >
            <ToggleGlyph on={online} disabled={!mpConfigured} />
          </button>
        </div>
      </div>
      ...
      <div className={`rounded-2xl p-5 border transition-all ${cashOnDelivery ? 'bg-zinc-900 border-zinc-800' : 'bg-orange-900/10 border-orange-800/40'}`}>
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={() => setCashOnDelivery(v => !v)}
      ...
      <Button onClick={handleSave} disabled={saving}
        className="w-full h-12 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold rounded-xl flex items-center justify-center gap-2">
        <SaveGlyph saving={saving} /> Salvar Configurações
      </Button>
    </div>
  );
}
...
function LinksTab() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setLinks(await getAdminExternalLinks());
    } finally {
      setLoading(false);
    }
  };
  ...
  const openNew = () => { setEditLink(null); setLabel(''); setUrl(''); setShowForm(true); setError(''); };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Form always mounted — show/hide via CSS to keep a stable sibling tree. */}
      <div className={`bg-zinc-900 border border-amber-500/30 rounded-xl p-4 space-y-3 ${showForm ? '' : 'hidden'}`}>
        ...
        <div className="flex gap-2">
          <Button onClick={handleSave} disabled={saving}
            ...
            <SaveGlyph saving={saving} /> Salvar
          </Button>
          ...
          >
            ...
            <div className="flex gap-1.5 shrink-0">
              <button
                ...
                className={`transition-colors ${l.active ? 'text-amber-500' : 'text-zinc-600'}`}
              >
                <ToggleGlyph on={l.active} />
              </button>
...
function WhatsappTab() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (alive) setNumber(s.number);
      } catch {
        if (alive) setError('Não foi possível carregar o WhatsApp');
      } finally {
        if (alive) setLoading(false);
      }
    })();
  ...
  return (
    <div className="space-y-5">
      ...
      <Button onClick={handleSave} disabled={saving}
        className="w-full h-12 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold rounded-xl flex items-center justify-center gap-2">
        <SaveGlyph saving={saving} /> Salvar Número
      </Button>
    </div>
  );
}
...
export default function SettingsHub() {
  ...
  const tabs: { id: Tab; label: string; icon: typeof CreditCard }[] = [
    ...
    { id: 'preparo', label: 'Tempo de Preparo', icon: Clock },
    { id: 'links', label: 'Links Externos', icon: LinkIcon },
    { id: 'whatsapp', label: 'WhatsApp', icon: MessageCircle },
    { id: 'ruas', label: 'Ruas de Entrega', icon: MapPin },
  ];
    ...
    <div className="min-h-screen bg-[#0a0a0a] pb-24">
      <header className="sticky top-0 z-40 bg-zinc-950/95 backdrop-blur border-b border-zinc-800 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <button type="button" onClick={handleLogout} className="p-2 text-zinc-400 hover:text-red-400 transition-colors">
            <LogOut size={20} />
          </button>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        {/* Keep all tabs mounted; toggle with CSS only — avoids remount/insertBefore on switch + save. */}
        <div className={tab === 'pagamento' ? '' : 'hidden'}><PaymentTab /></div>
        <div className={tab === 'preparo' ? '' : 'hidden'}><PrepTimeTab /></div>
        <div className={tab === 'links' ? '' : 'hidden'}><LinksTab /></div>
        <div className={tab === 'whatsapp' ? '' : 'hidden'}><WhatsappTab /></div>
        <div className={tab === 'ruas' ? '' : 'hidden'}><RuasTab /></div>
      </main>

      ...
      {product && (
        ...
        >
          <motion.div
            ...
            className="w-full sm:max-w-md max-h-[92vh] bg-zinc-950 rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col border border-zinc-800"
            onClick={e => e.stopPropagation()}
          >
            {/* Media */}
            <div className="relative w-full h-56 shrink-0 bg-zinc-900">
              {product.videoUrl ? (
                <video
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              <div>
                <h2 className="text-white font-black uppercase text-xl leading-tight">{product.name}</h2>
            ...
            {/* Sticky bottom bar */}
            <div className="shrink-0 border-t border-zinc-800 bg-zinc-950 px-5 py-4 space-y-3">
              <div className="flex items-center justify-center gap-4 bg-zinc-900 border border-zinc-800 rounded-xl py-2">
                <button
                  onClick={() => setQuantity(q => Math.max(1, q - 1))}
                  className="w-9 h-9 flex items-center justify-center bg-zinc-800 text-white rounded-lg hover:bg-zinc-700 active:scale-95 transition-transform"
                >
                  <Minus size={16} />
                </button>
                ...
                <button
                  ...
                  className="w-9 h-9 flex items-center justify-center bg-amber-500 text-zinc-950 rounded-lg hover:brightness-110 active:scale-95 transition-transform"
                >
                  <Plus size={16} />
                </button>
              </div>
              <button
                ...
                className="w-full h-13 py-3.5 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 transition-colors active:scale-[0.98]"
              >
